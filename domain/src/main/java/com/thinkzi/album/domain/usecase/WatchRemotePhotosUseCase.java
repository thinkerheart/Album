...
/**
 * provide UseCase(Clean Architecture) to watch photos from server
 * */
public final class WatchRemotePhotosUseCase extends ObservableUseCase<Photo, Void> {

    private final IPhotoRepository _iPhotoRepository;

    @Inject
    public WatchRemotePhotosUseCase(ThreadExecutor _threadExecutor, PostExecutionThread _postExecutionThread, IPhotoRepository _iPhotoRepository) {
    ...
    protected Observable<Photo> buildObservableUseCase(Void _params) {
        return _iPhotoRepository.getRemotePhotos();
    }
}
